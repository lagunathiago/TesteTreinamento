...
describe("Teste - Login", () => {
  ...
  context("Criando Treinamento", { testIsolation: false }, () => {
    ...
    it('Cria o primeiro Treinamento com campos', () => {
      ...
      cy.get('label.thumb-placeholder[aspect="square"] input[type="file"]', { timeout: 60000 })
        .should('exist')
        .selectFile('cypress/fixtures/Primeiro Campo.png', { force: true });
    ...
    it("Conteúdo - Avaliação (uma por página)", () => {
      ...
      cy.contains(".ui-select-container", "Escolha uma avaliação", { timeout: 60000 })
        .should("be.visible")
        .click();
    ...
    it('Criando turma do treinamento', () => {
      ...
      cy.get("#className", { timeout: 60000 })
        .should('be.visible')
        .type("Turma Teste Automação Turma");

      cy.get('.column > :nth-child(1) > .icon-checkbox', { timeout: 60000 })
        .should('be.visible')
        .click();

      cy.get('.navigation-controls > .ml-20', { timeout: 60000 })
        .should('be.visible')
        .click();
      ...
      cy.get('tr.ng-scope > :nth-child(4) > .middle > .btn', { timeout: 60000 })
        .should('be.visible')
        .click();

      cy.get('.step2 > .permission-select > [ng-show="showUser"] > .column > .multiselect > .border > .ui-select-match > .btn-default', { timeout: 60000 })
        .should('be.visible')
        .type("Aluno");

      cy.get('.ui-select-dropdown', { timeout: 60000 })
        .should('be.visible');
      ...
      cy.contains('button', 'Adicionar', { timeout: 60000 })
        .should('be.visible')
        .click();
    });
    ...
    it('Adicinando os campos peronalizado', () => {
      ...
      cy.get('.add-content > .end > .btn-swipe-accent', { timeout: 60000 })
        .should('be.visible')
        .click();
    ...
    it('Adicinando os campos peronalizado', () => {
      ...
      cy.contains('span.ui-select-toggle', 'Selecione um campo personalizado', { timeout: 60000 })
        .should('be.visible')
        .click({ force: true });

      cy.contains('span.ui-select-choices-row-inner', 'Personalizado - 01', { timeout: 60000 })
        .should('be.visible')
        .click({ force: true });
      ...
      cy.contains('span.ui-select-choices-row-inner', 'Personalizado - 02', { timeout: 60000 })
        .should('be.visible')
        .click({ force: true });
    ...
    it('Meu Cadastro', () => {
      ...
      cy.get('input[placeholder="Personalizado - 01"]', { timeout: 60000 })
        .should('exist');

      cy.get('input[placeholder="Personalizado - 02"]', { timeout: 60000 })
        .should('exist');
    ...
    it('Verefica que o "Selecionar um campo personalizado esteja desabilitado"', () => {
      cy.get('.multiselect', { timeout: 60000 })
        .should('not.be.enabled');
    });

    it('Verifica se a lixeira não está na página', () => {
      cy.get('button[title="Remover"]', { timeout: 60000 })
        .should('not.exist');
    });

    it('Verifica se o botão de não está na página', () => {
      cy.get('button[title="Editar"]', { timeout: 60000 })
        .should('not.exist');
    });
    ...
    it('Verefica se os Campos Perosnalizados criados anteriormente estejam na página', () => {
      cy.contains('td', 'Personalizado - 01', { timeout: 60000 })
        .should('be.visible');

      cy.contains('td', 'Personalizado - 02', { timeout: 60000 })
        .should('be.visible');

      cy.contains('td.ng-binding', 'Texto', { timeout: 60000 })
        .should('be.visible');
    });
  });
});